---
title: "NEOSTAR.subpopulation_comparison"
...
ll=load("report/SC307.RData/neostar.final.RData")
...
pp_test = function(subpop, pop, arm=NA, group=NA, ssample=NA, cmp=c("Arm"="C_vs_D"), plot_bar=T, DEG=F){
  ...
  folder = sprintf("report/SC307.report_13.neostar.comparison/%s.%s_%s", pop, names(cmp), gsub("/", "_", cmp, fixed=T))
...
DEG_test = function(p1, p2, cmpname, folder, name, subsample=0, FDR=0.05, log2FC=1){
  ...
  folder = sub("report/SC307.report_13.neostar.comparison", "report/SC307.report_13.neostar.comparison/DEG_analysis", folder, fixed=T)
  dir.create(folder, showWarnings=F)
...
vt_cor = function(subpop, outofpop){
  folder = sprintf("report/SC307.report_13.neostar.comparison/%s.VT_correlation", outofpop)
  dir.create(folder, showWarnings=F)
