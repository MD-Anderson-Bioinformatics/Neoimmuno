---
title: "NEOPSTAR.lineage_identification"
...
ll = load("report/SC307.RData/neostar.final_no_prolif.RData")
data = as.matrix(sc@assays$RNA@data)
sdata = as.matrix(sc@assays$integrated@scale.data)
load("report/SC307.RData/neostar.cell_annotation.RData")
...
pdf(sprintf("report/SC307.report_12.neostar.figure/Major_lineage.umap_plot.pdf"), width=8, height=8)
...
pdf(sprintf("report/SC307.report_12.neostar.figure/Major_lineage.umap_plot1.pdf"), width=8, height=8)
...
pdf("report/SC307.report_12.neostar.figure/singleR/annotation2.pdf", width=9, height=8)
...
feature_plot_png2("Human_Prime_Cell", sr1$scores, folder="report/SC307.report_12.neostar.figure/singleR") 
feature_plot_png2("Immune_Cell", sr2$scores, folder="report/SC307.report_12.neostar.figure/singleR") 

```

## project to previous cluster
```{r}
cell_cluster1 = gsub(".C", ".", gsub("cluster", "C", gsub("neostar.", "", cell_cluster[colnames(sc)], fixed=T), fixed=T), fixed=T)
cid2 = sapply(cell_cluster1, function(x) strsplit(x, ".", fixed=T)[[1]][1])
cids2 = unique(cid2)
cid2_col = rainbow(length(cids2))
names(cid2_col) = cids2

pdf(sprintf("report/SC307.report_12.neostar.figure/Cluster_projection.umap_plot.pdf"), width=8, height=8)
...
df = bubble_plot_landscape("Major_lineage", markers, cid, folder=sprintf("report/SC307.report_12.neostar.figure/"), groups=c("Lymphoid"="Lymphoid", "Myeloid"="Myeloid", "Stromal"="Stromal", "Epithelial"="Epithelial"), size=c(10, 2.5), lim=1)
```
...
umap0 = umap
cc = "Epithelial"
ll = load(sprintf("report/SC307.RData/neostar.%s.RData", cc)); gc()
data = as.matrix(sc@assays$RNA@data)
sdata = as.matrix(sc@assays$integrated@scale.data)
load("report/SC307.RData/neostar.cell_annotation.RData")
...
pdf(sprintf("report/SC307.report_12.neostar.figure/%s.umap_plot.pdf", cc), width=8, height=8)
plot(umap[, 1], umap[, 2], pch=20, xlab="UMAP1", ylab="UMAP2", cex.axis=1.5, cex.lab=1.5, cex.main=1.5, cex=0.3, font.axis=1.5, font.lab=1.5, main=sprintf("Total cells (n = %d)", nrow(umap)), las=1, col=cell_col[cid], xlim=c(min(umap[,1]), max(umap[, 1])))
pos = 0
for(c in cids){
...
df = bubble_plot_landscape(cc, markers[[cc]], cid, folder=sprintf("report/SC307.report_12.neostar.figure/"), size=sizes[[cc]], groups=groups[[cc]], lim=1)
```
...
ll = load("report/SC307.RData/neostar.final_no_prolif.RData")
umap0 = umap
ll = load("report/SC307.RData/neostar.Lymphoid.RData")
umap1 = umap
cc = "CD8+ T"
ll = load(sprintf("report/SC307.RData/neostar.Lymphoid.%s.RData", cc))
data = as.matrix(sc@assays$RNA@data)
sdata = as.matrix(sc@assays$integrated@scale.data)
load("report/SC307.RData/neostar.cell_annotation.RData")
cells = colnames(sc)
...
pdf(sprintf("report/SC307.report_12.neostar.figure/Lymphoid.%s.umap_plot.pdf", cc), width=8, height=8)
...
df = bubble_plot_landscape(cc, markers[[cc]], cid, folder=sprintf("report/SC307.report_12.neostar.figure/Lymphoid."), size=sizes[[cc]], groups=groups[[cc]], lim=1)
